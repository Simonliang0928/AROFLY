...
#define DEFAULT_HEARTRATE_PERIOD                        50
...
static void HeartRate_taskFxn(UArg a0, UArg a1)
{
  extern void FBM320_init(void);
  extern void BMA250E_init(void);
  extern void FBMSensor_Start(void);
...
//     }  
  ...
  for(;;)
  {
    ...
    // Baroimeter sensor start.
    if (events & TEMPERATURE_PERIODIC_EVT)
    {
      events &= ~TEMPERATURE_PERIODIC_EVT;
...
//    *p++ = heartRateBpm;
    {
    	...
    	*p++ = HI_UINT16(Acc.z);
    	*p++ = LO_UINT16(Acc.z);
    	*p++ = (Barometric_Pressure>>24)&0xFF;
    	*p++ = (Barometric_Pressure>>16)&0xFF;
    	*p++ = (Barometric_Pressure>>8)&0xFF;
    	*p++ = (Barometric_Pressure>>0)&0xFF;
	}	
...
static void HeartRate_measPerTask(void)
{
  extern void BMASensor_get(BMA250E_ACC_TMP *ptr);
  extern uint32_t FBMSensor_get();
  ...
  if (gapProfileState == GAPROLE_CONNECTED)
  {
    ...
    // Restart timer.
    Util_startClock(&measPerClock);
    Barometric_Pressure = FBMSensor_get();
    BMASensor_get(&Acc);
  }
}
...
void Sensor_clockHandler(UArg arg){
	HeartRate_clockHandler(arg);
}	
